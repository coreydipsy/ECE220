// Corey Yu kaiyiyu2
//this program is a game that allows user to input number to generate random number and have 12 times to guess it
// the program will output how many perfect matched charcter and misplaced one and output to the user
...
int main ()
{
    ...
    do {
		...
		if (NULL == fgets (buf, 200, stdin)) { // store the string into buf, only read less than 200, read from stdin
			printf ("\nProgram terminated.\n"); // if reads null print terminate the program
			return 3; 
		}
    } while (!set_seed (buf));  // while set_seed function output 0

    //Call start game to get solution
    start_game (&solution[0], &solution[1], &solution[2], &solution[3]);// input address of soloution to start_game

    //Go until 12 guesses, or user wins
    for (number = 1; 12 >= number; number++) {// count 12 times
	printf ("Guess %d\n", number);
	do {
